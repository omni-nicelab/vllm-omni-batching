...
def _flash_attn_fwd(
    ...
    def convert_from_dlpack_fp8(x, leading_dim=None, int8: bool = False):
    ...
    int8 = True if q_fp8.dtype == torch.int8 else False
    q_fp8_tensor = convert_from_dlpack_fp8(q_fp8, leading_dim=3, int8=int8)
    k_fp8_tensor = convert_from_dlpack_fp8(k_fp8, leading_dim=3, int8=int8)
    vt_fp8_tensor = convert_from_dlpack_fp8(vt_fp8, leading_dim=1, int8=int8)

    q_scale_tensor = from_dlpack(q_scale.detach(), assumed_align=16).mark_layout_dynamic(leading_dim= 1)
    k_scale_tensor = from_dlpack(k_scale.detach(), assumed_align=16).mark_layout_dynamic(leading_dim= 1)
    v_scale_tensor = from_dlpack(v_scale.detach(), assumed_align=16).mark_layout_dynamic(leading_dim=v_scale.ndim - 1) if v_scale is not None else None 
...
class FlashAttnFunc(torch.autograd.Function):
    ...
    def backward(ctx, dout, *args):
        assert False, "Not implemented: we never call backward"
...
# Self-reference for `from .interface import interface` style imports
import sys
