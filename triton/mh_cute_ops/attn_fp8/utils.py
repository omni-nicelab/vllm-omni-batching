...
@cute.jit
def permute_P(P: cute.Tensor):
    # # P has shape ((2, 2, N / 8), MMA_M, MMA_N)
    print(" ------------ P shape: ", P.shape, "stride: ", P.stride)
    # # TODO optimize performance
    shape_02 = P.shape[0][2]
    shape_1 = P.shape[1]
    shape_2 = P.shape[2]

    # for i in cutlass.range_constexpr(shape_2):
    #     for j in cutlass.range_constexpr(shape_1):
    #         for k in cutlass.range_constexpr(0, shape_02, 2):
    #             for l in cutlass.range_constexpr(2):
    #                 tmp = P[((l,0,k+1),j,i)]
    #                 P[((l,0,k+1),j,i)] = P[((l,1,k),j,i)] 
    #                 P[((l,1,k),j,i)] = tmp

    tidx = cute.arch.thread_idx()[0] % 4
    for i in cutlass.range_constexpr(shape_2):
        for j in cutlass.range_constexpr(shape_1):
            for k in cutlass.range_constexpr(0, shape_02, 2):
                # P[((0,0,k),j,i)] = tidx * 0.4 + 0.1
                # P[((1,0,k),j,i)] = tidx * 0.4 + 0.2
                # P[((0,1,k),j,i)] = tidx * 0.4 + 0.3
                # P[((1,1,k),j,i)] = tidx * 0.4 + 0.4
                # P[((0,0,k+1),j,i)] = tidx * 0.4 + 0.1
                # P[((1,0,k+1),j,i)] = tidx * 0.4 + 0.2
                # P[((0,1,k+1),j,i)] = tidx * 0.4 + 0.3
                # P[((1,1,k+1),j,i)] = tidx * 0.4 + 0.4
                P[((0,0,k),j,i)] =  0.1 + tidx * 0.2
                P[((1,0,k),j,i)] =  0.2 + tidx * 0.2
                P[((0,1,k),j,i)] =  0.1 + tidx * 0.2
                P[((1,1,k),j,i)] =  0.2 + tidx * 0.2 
                P[((0,0,k+1),j,i)] =  0.1 + tidx * 0.2
                P[((1,0,k+1),j,i)] =  0.2 + tidx * 0.2
                P[((0,1,k+1),j,i)] =  0.1 + tidx * 0.2
                P[((1,1,k+1),j,i)] =  0.2 + tidx * 0.2 
...
@cute.jit
def convert_layout_acc_frgA_fp8(acc_layout: cute.Layout) -> cute.Layout:
    # For back to back gemm, convert layout of acc0 to gemm 1 accept layout.
    # TODO: Sm90 FP8
    # from ((2, 2, N / 8), MMA_M, MMA_N) to ((4, 2, 2), MMA_M, (N / 32, MMA_N))
    if const_expr(cute.rank(acc_layout.shape[0]) == 3):  # Sm90
        l = cute.logical_divide(
            acc_layout, ((None, None, 4), None, None)
        )  # ((2, 2, (4, N / 32)), MMA_M, MMA_N)
...
@dsl_user_op
def cvt_f8x2_f16(a: float | Float32, b: float | Float32, to_dtype: Type, *, loc=None, ip=None) -> cutlass.Int16:
    # assert to_dtype in [cutlass.BFloat16, cutlass.Float16], "to_dtype must be BFloat16 or Float16"
    return cutlass.Int16(
        llvm.inline_asm(
            T.i16(),
            [Float32(a).ir_value(loc=loc, ip=ip), Float32(b).ir_value(loc=loc, ip=ip)],
            f"cvt.rn.satfinite.e4m3x2.f32 $0, $2, $1;",
            "=h,f,f",
            has_side_effects=False,
            is_align_stack=False,
            asm_dialect=llvm.AsmDialect.AD_ATT,
...
@dsl_user_op
def cvt_u8x2_f32_magic_0_127(a: Float32, b: Float32, *, loc=None, ip=None) -> cutlass.Int16:
    out_i16 = llvm.inline_asm(
        T.i16(),
        [Float32(a).ir_value(loc=loc, ip=ip), Float32(b).ir_value(loc=loc, ip=ip)],
        "{\n\t"
        ".reg .f32 ta, tb;\n\t"
        ".reg .s32 ia, ib, tmp;\n\t"
        "add.f32 ta, $1, 0f4B400000;\n\t"
        "add.f32 tb, $2, 0f4B400000;\n\t"
        "mov.b32 ia, ta;\n\t"
        "mov.b32 ib, tb;\n\t"
        "prmt.b32 tmp, ia, ib, 0x8840;\n\t"
        "cvt.u16.u32 $0, tmp;\n\t"
        "}\n",
        "=h,f,f",
        has_side_effects=False,
    ...
    return cutlass.Int16(out_i16)

def cvt_u8x1_f32_magic_0_127(a: Float32, *, loc=None, ip=None) -> cutlass.Int8:
    out_i8 = llvm.inline_asm(
        T.i8(),  # 返回类型改为 i8
        [Float32(a).ir_value(loc=loc, ip=ip)],
        "{\n\t"
        ".reg .f32 ta;\n\t"
        ...
        "cvt.u16.u32 $0, ia;\n\t"
        "}\n",
        "=h,f", # =h 表示输出到 16-bit 寄存器 (对于 i8 也是兼容的), f 为 float 输入
        has_side_effects=False,
        is_align_stack=False,
        asm_dialect=llvm.AsmDialect.AD_ATT,
    )
    return cutlass.Int8(out_i8)

@dsl_user_op
def cvt_u8x4_f32_magic_0_127(a: Float32, b: Float32, c: Float32, d: Float32, *, loc=None, ip=None) -> cutlass.Int32:
    out_i32 = llvm.inline_asm(
        T.i32(),
        [
            Float32(a).ir_value(loc=loc, ip=ip),
            Float32(b).ir_value(loc=loc, ip=ip),
        ...
        "add.f32 ta, $1, 0f4B400000;\n\t"
        "add.f32 tb, $2, 0f4B400000;\n\t"
        "add.f32 tc, $3, 0f4B400000;\n\t"
        "add.f32 td, $4, 0f4B400000;\n\t"
        "mov.b32 ia, ta;\n\t"
        "mov.b32 ib, tb;\n\t"
        "mov.b32 ic, tc;\n\t"
        "mov.b32 id, td;\n\t"
        "prmt.b32 pab, ia, ib, 0x8840;\n\t"
        "prmt.b32 pcd, ic, id, 0x8840;\n\t"
        "prmt.b32 $0, pab, pcd, 0x5410;\n\t"
        # "mov.b32 $0, out;\n\t"
        "}\n",
        "=r,f,f,f,f",
        has_side_effects=False,
    ...
    return cutlass.Int32(out_i32)


@dsl_user_op
def round_fp32_rn(a: float | Float32, loc=None, ip=None) -> cutlass.Float32:
    return Float32(
        llvm.inline_asm(
            T.f32(),
            [Float32(a).ir_value(loc=loc, ip=ip)],
...
# Self-reference for `from .utils import utils` style imports
import sys
