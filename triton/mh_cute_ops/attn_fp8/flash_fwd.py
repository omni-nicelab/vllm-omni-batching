...
import quack.reduce as quack_utils

try:
    from .ampere_helpers import ampere_helpers as sm80_utils
    from .hopper_helpers import hopper_helpers as sm90_utils
    from .utils import utils
    from .copy_utils import copy_utils
...
# Environment variable to control printing of compiler constants
_PRINT_CONSTANTS = os.getenv("FLASH_ATTN_PRINT_CONSTANTS", "0").lower() in ("1", "true", "yes")
_PRINT_CONSTANTS = False
...
class FlashAttentionForwardBase:
    ...
    def __init__(
        ...
        # Print constants if enabled
        if _PRINT_CONSTANTS and self.arch == 90:
            print("\n" + "=" * 80)
            print("FlashAttentionForwardBase Constants (from __init__)")
            print("=" * 80)
            print(f"  arch: {self.arch}")
            print(f"  dtype: {self.dtype}")
            print(f"  head_dim: {head_dim}")
            print(f"  head_dim_v: {head_dim_v}")
            print(f"  tile_hdim: {self.tile_hdim} (padded from {head_dim})")
            print(f"  tile_hdimv: {self.tile_hdimv} (padded from {head_dim_v})")
            print(f"  same_hdim_kv: {self.same_hdim_kv}")
            print(f"  check_hdim_oob: {self.check_hdim_oob}")
            print(f"  check_hdim_v_oob: {self.check_hdim_v_oob}")
            print(f"  qhead_per_kvhead: {self.qhead_per_kvhead}")
            print(f"  is_causal: {self.is_causal}")
            print(f"  is_local: {self.is_local}")
            print(f"  pack_gqa: {self.pack_gqa}")
            ...
            print(f"  Q_in_regs: {self.Q_in_regs}")
            print(f"  vec_size: {self.vec_size}")
    ...
    def _setup_attributes(self):
        ...
        # Print copy and layout constants if enabled
        if _PRINT_CONSTANTS and self.arch == 90:
            print("\n" + "=" * 80)
            print("FlashAttentionForwardSm90 Copy & Layout Constants (from _setup_attributes)")
            print("=" * 80)
            print(f"  universal_copy_bits: {universal_copy_bits}")
            print(f"  async_copy_elems: {async_copy_elems} (128 bits / {self.dtype.width} bits)")
            print(f"  tQK_shape_dim_1: {tQK_shape_dim_1}")
            print(f"  tV_shape_dim_1: {tV_shape_dim_1}")
            print(f"  tQ_layout: ({self.num_Q_load_threads // tQK_shape_dim_1}, {tQK_shape_dim_1})")
            print(f"  tK_layout: ({self.num_producer_threads // tQK_shape_dim_1}, {tQK_shape_dim_1})")
            print(f"  tV_layout: ({self.num_producer_threads // tV_shape_dim_1}, {tV_shape_dim_1})")
            print(f"  tO_layout: ({self.num_epilogue_threads // tV_shape_dim_1}, {tV_shape_dim_1})")
            print(f"  Tile shapes:")
            print(f"    Q: ({self.tile_m}, {self.tile_hdim})")
            print(f"    K: ({self.tile_n}, {self.tile_hdim}, {self.num_stages})")
            print(f"    V: ({self.tile_n}, {self.tile_hdimv}, {self.num_stages})")
            print(f"    O: ({self.tile_m}, {self.tile_hdimv})")
            if self.sP_layout is not None:
                print(f"    P: ({self.tile_m}, {self.tile_n})")
...
class FlashAttentionForwardSm90(FlashAttentionForwardBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Print SM90 specific constants if enabled
        if _PRINT_CONSTANTS:
            print("\n" + "=" * 80)
            print("FlashAttentionForwardSm90 Constants (from __init__)")
            print("=" * 80)
            print(f"  intra_wg_overlap: {self.intra_wg_overlap}")
            print(f"  mma_pv_is_rs: {self.mma_pv_is_rs}")
    ...
    def _get_smem_layout_atom(self):
        ...
        if not self.mma_pv_is_rs:
            ...
            sP_layout_atom = warpgroup.make_smem_layout_atom(
                sm90_utils_basic.get_smem_layout_atom(
                    LayoutEnum.ROW_MAJOR, cute.Float8E4M3FN, self.tile_n
                ),
                cute.Float8E4M3FN,
            )
    ...
    def _get_tiled_mma(self):
        ...
        if const_expr(self.int8 == True):
            dtype_qk = cute.Int8
            dtype_acc = cute.Int32
        else:
            dtype_qk = cute.Float8E4M3FN
            dtype_acc = cute.Float32
        ...
        # Print MMA constants if enabled
        if _PRINT_CONSTANTS:
            print("\n" + "=" * 80)
            print("FlashAttentionForwardSm90 MMA Constants (from _get_tiled_mma)")
            print("=" * 80)
            print(f"  tiled_mma_qk:")
            print(f"    atom_layout_mnk: {atom_layout_mnk}")
            print(f"    tiler_mn: (64, {self.tile_n})")
            print(f"  tiled_mma_pv:")
            print(f"    atom_layout_mnk: {atom_layout_mnk}")
            print(f"    tiler_mn: (64, {self.tile_hdimv})")
            print(f"    a_source: {'RMEM' if self.mma_pv_is_rs else 'SMEM'}")
            print(f"  tiled_mma_pv_rs:")
            print(f"    atom_layout_mnk: {atom_layout_mnk}")
            print(f"    tiler_mn: (64, {self.tile_hdimv})")
            print(f"    a_source: RMEM")
    ...
    def _get_shared_storage_cls(self):
        ...
        sV_alignment = 128
        sQ_struct, sK_struct, = [
            cute.struct.Align[cute.struct.MemRange[cute.Int8, cute.cosize(layout)], alignment]
            for layout, alignment in zip(
                (self.sQ_layout, self.sK_layout),
                (sQ_alignment, sK_alignment, ),
            )
        ]
        sV_struct = cute.struct.Align[cute.struct.MemRange[cute.Float8E4M3FN, cute.cosize(self.sV_layout)], 128]
        sQscale_struct = cute.struct.Align[cute.struct.MemRange[cute.Float32, cute.cosize(self.sQscale_layout)], 128]
        sKscale_struct = cute.struct.Align[cute.struct.MemRange[cute.Float32, cute.cosize(self.sKscale_layout)], 128]
        sVscale_struct = cute.struct.Align[cute.struct.MemRange[cute.Float32, cute.cosize(self.sVscale_layout)], 128]

        cosize_sQV = max(cute.cosize(self.sQ_layout), cute.cosize(self.sV_layout))
        sQV_struct = cute.struct.Align[cute.struct.MemRange[self.dtype, cosize_sQV], 1024]
        cosize_sP = cute.cosize(self.sP_layout) if const_expr(self.sP_layout is not None) else 0
        sP_struct = cute.struct.Align[cute.struct.MemRange[cutlass.Float8E4M3FN, cosize_sP], 1024]
        sPscale_struct = cute.struct.Align[cute.struct.MemRange[cutlass.Float32, cosize_sP // 128], 128]
        ...
        # Print shared memory alignment constants if enabled
        if _PRINT_CONSTANTS:
            dtype_size = 2 if self.dtype == cutlass.Float16 else 2  # float16/bfloat16
            smem_Q = self.tile_m * self.tile_hdim * dtype_size
            smem_K = self.tile_n * self.tile_hdim * self.num_stages * dtype_size
            smem_V = self.tile_n * self.tile_hdimv * self.num_stages * dtype_size
            smem_O = self.tile_m * self.tile_hdimv * dtype_size
            total_smem = smem_Q + smem_K + smem_V + smem_O
            print("\n" + "=" * 80)
            print("FlashAttentionForwardSm90 Shared Memory Constants (from _get_shared_storage_cls)")
            print("=" * 80)
            print(f"  sQ_alignment: {sQ_alignment} bytes")
            print(f"  sK_alignment: {sK_alignment} bytes")
            print(f"  sV_alignment: {sV_alignment} bytes")
            print(f"  sQV_struct alignment: 1024 bytes")
            if self.sP_layout is not None:
                print(f"  sP_struct alignment: 1024 bytes")
            print(f"  Shared memory usage:")
            print(f"    Q: {smem_Q:,} bytes ({self.tile_m} * {self.tile_hdim} * {dtype_size})")
            print(f"    K: {smem_K:,} bytes ({self.tile_n} * {self.tile_hdim} * {self.num_stages} * {dtype_size})")
            print(f"    V: {smem_V:,} bytes ({self.tile_n} * {self.tile_hdimv} * {self.num_stages} * {dtype_size})")
            print(f"    O: {smem_O:,} bytes ({self.tile_m} * {self.tile_hdimv} * {dtype_size})")
            print(f"    Total: {total_smem:,} bytes ({total_smem / 1024:.2f} KB)")
            print(f"  Memory barrier structure:")
            print(f"    mbar_ptr_QO: 2 entries (for Q and O)")
            print(f"    mbar_ptr_K: {self.num_stages * 2} entries")
            print(f"    mbar_ptr_V: {self.num_stages * 2} entries")
        ...
        class SharedStorageQKV:
            ...
            sV: sV_struct
            sVscale: sVscale_struct
            sQ: sQ_struct
            sK: sK_struct
            sP: sP_struct
            sQscale: sQscale_struct
            sKscale: sKscale_struct
            # sPscale: sPscale_struct
    ...
    def __call__(
        ...
        mQ_fp8: cute.Tensor, # (b, s_q, h, d)
        mQ_scale: cute.Tensor, # (b, s_q, h)
        mK: cute.Tensor,  # (b_k, s_k, h_k, d) or (total_k, h_k, d) if there is cu_seqlens_k or (num_pages, page_size, h_k, d) if there is page_table
        mK_fp8: cute.Tensor, # (b_k, s_k, h_k, d)
        mK_scale: cute.Tensor, # (b_k, s_k, h_k)
        mV0: cute.Tensor,  # (b_k, s_k, h_k, dv) or (total_k, h_k, dv) if there is cu_seqlens_k or (num_pages, page_size, h_k, dv) if there is page_table
        mVt_fp8: cute.Tensor, # b_k, h_k, dv, s_k
        mV_scale: cute.Tensor | None, # b_k, h_k, s_k
        ...
        self._check_type(
            *(
                ...
                for t in (mQ, mK, mV0, mO, mLSE, mCuSeqlensQ, mCuSeqlensK, mSeqUsedQ, mSeqUsedK)
            )
        )
        ...
        mQ, mK, mV, mO = [
            ...
            for t in (mQ, mK, mV0, mO)
        ]
        # print("mQ stride: ", mQ.stride)
        # print("mV stride: ", mV.stride)
        # print("mV0 stride: ", mV0.stride)
        # print("mVt_fp8 shape: ", mVt_fp8.shape, "stride: ", mVt_fp8.stride)
        ...
        Vscale_layout_transpose = [1, 3, 2, 0]
        mV_scale = utils.select(mV_scale, Vscale_layout_transpose)
        ...
        # Print runtime constants if enabled
        # if _PRINT_CONSTANTS and False:
        #     print("\n" + "=" * 80)
        #     print("FlashAttentionForwardSm90 Runtime Constants (from __call__)")
        #     print("=" * 80)
        #     print(f"  num_mma_threads: {self.num_mma_threads}")
        #     print(f"  num_threads_per_warp_group: {self.num_threads_per_warp_group}")
        #     print(f"  num_mma_warp_groups: {self.num_mma_warp_groups}")
        #     print(f"  num_threads (recalculated): {self.num_threads}")
        #     print(f"  num_producer_threads: {self.num_producer_threads}")
        #     print(f"  num_Q_load_threads: {self.num_Q_load_threads}")
        #     print(f"  num_epilogue_threads: {self.num_epilogue_threads}")
        #     print(f"  num_mma_regs: {self.num_mma_regs}")
        #     print(f"  num_producer_regs: {self.num_producer_regs}")
        #     print(f"  use_block_sparsity: {self.use_block_sparsity}")
        #     print(f"  use_scheduler_barrier: {self.use_scheduler_barrier}")
        #     print(f"  use_tma_Q: {self.use_tma_Q}")
        #     print(f"  use_tma_O: {self.use_tma_O}")
        ...
        self.sV_layout = sm90_utils.make_smem_layout(mV.element_type, LayoutEnum.COL_MAJOR, (self.tile_n, self.tile_hdimv), self.num_stages)
        self.sVscale_layout = cute.make_layout(((1, self.tile_hdimv), self.num_stages), stride=((1,1), self.tile_hdimv))

        self.sQscale_layout = cute.make_layout(((1, self.tile_m), 1), stride=((1,1), self.tile_m))
        self.sKscale_layout = cute.make_layout(((1, self.tile_n), self.num_stages), stride=((1,1), self.tile_n))

        # define layout for sVscale, 3 steps:
        # 1 define smem layout atom type
        # 2 define atom
        # 3 define layout
        # sVscale_layout_atom_kind = cute.nvgpu.warpgroup.SmemLayoutAtomKind.K_INTER
        # sVscale_layout_atom = cute.nvgpu.warpgroup.make_smem_layout_atom(sVscale_layout_atom_kind, cute.Float32)


        self.sP_layout = None
        self.sPscale_layout = None
        if const_expr(self.mma_pv_is_rs):
            self.sP_layout = sm90_utils.make_smem_layout(
                cute.Float8E4M3FN, LayoutEnum.ROW_MAJOR, (self.tile_m, self.tile_n)
            )
            # self.sPscale_layout = cute.make_layout((self.tile_m, self.tile_n//128),stride=(self.tile_n//128, 1))
        ...
        self.tma_copy_bytes = {
            ...
            for name, mX, layout in [
                ...
                ("K", mK, self.sK_layout),
                ("V", mV, self.sV_layout),
                # ("Vscale", mV_scale, self.sVscale_layout),
            ]
        }
        self.tma_copy_bytes["Vscale"] = cute.size_in_bytes(mV_scale.element_type, cute.select(self.sVscale_layout, mode=[0]))
        self.tma_copy_bytes["Qscale"] = cute.size_in_bytes(mQ_scale.element_type, cute.select(self.sQscale_layout, mode=[0]))
        self.tma_copy_bytes["Kscale"] = cute.size_in_bytes(mK_scale.element_type, cute.select(self.sKscale_layout, mode=[0]))
        ...
        # MING check
        tma_atom_Qscale, tma_tensor_Qscale = cpasync.make_tiled_tma_atom(
            gmem_tiled_copy_Q,
            mQ_scale,
            # cute.select(self.sVscale_layout, mode=[0]),
            # cute.slice_(self.sVscale_layout, (None, 0)),
            cute.make_layout(( 1, self.tile_m), stride=( 1,1)),
            cta_tiler=( 1,self.tile_m),
        )
        tma_atom_Kscale, tma_tensor_Kscale = cpasync.make_tiled_tma_atom(
            gmem_tiled_copy_KV,
            mK_scale,
            cute.make_layout(( 1, self.tile_n), stride=( 1,1)),
            cta_tiler=( 1,self.tile_n),
        )
        tma_atom_Vscale = None
        tma_tensor_Vscale = None
        tma_atom_Vscale, tma_tensor_Vscale = cpasync.make_tiled_tma_atom(
            gmem_tiled_copy_KV,
            mV_scale,
            # cute.select(self.sVscale_layout, mode=[0]),
            # cute.slice_(self.sVscale_layout, (None, 0)),
            cute.make_layout(( 1, self.tile_hdimv), stride=( 1,1)),
            cta_tiler=( 1,self.tile_hdimv),
        )
    ...
    def kernel(
        ...
        sQscale_layout: cute.Layout,
        sKscale_layout: cute.Layout,
        sVscale_layout: cute.Layout,
        sO_layout: cute.ComposedLayout,
        sP_layout: cute.ComposedLayout | None,
        sPscale_layout: cute.Layout | None,
        ...
        pipeline_k = pipeline.PipelineTmaAsync.create(
            ...
            tx_count=self.tma_copy_bytes["K"] + self.tma_copy_bytes["Kscale"],
            init_wait=False,
        )
        pipeline_v = pipeline.PipelineTmaAsync.create(
            ...
            producer_group=pipeline_kv_producer_group,
            consumer_group=pipeline_kv_consumer_group,
            tx_count=self.tma_copy_bytes["V"] + self.tma_copy_bytes["Vscale"],
        ...
        # sVscale = storage.sVscale.get_tensor(sVscale_layout)
    ...
    def load(
        ...
        sQscale: cute.Tensor,
        sKscale: cute.Tensor,
        sVscale: cute.Tensor | None,
        tma_atom_Q: cute.CopyAtom,
        tma_atom_K: cute.CopyAtom,
        tma_atom_V: cute.CopyAtom,
        tma_atom_Qscale: cute.CopyAtom | None,
        tma_atom_Kscale: cute.CopyAtom | None,
        tma_atom_Vscale: cute.CopyAtom | None,
        ...
        # if cute.arch.thread_idx()[0] == 0 and cute.arch.block_idx()[0] == 0 and cute.arch.block_idx()[1] == 0 and cute.arch.block_idx()[2] == 0:
        #     cute.printf("mVscale shape: %d, %d, %d, %d", mVscale.shape[0], mVscale.shape[1], mVscale.shape[2], mVscale.shape[3])
        ...
        if warp_idx_in_wg == 0:
            ...
            while work_tile.is_valid_tile:
                ...
                # mVscale_cur = seqlen.offset_batch_K(mVscale, batch_idx, dim=3)[None, None, head_idx_kv]
                # mV_cur = mV[None,None,head_idx_kv,batch_idx]
                mVscale_cur = mVscale[None, None, head_idx_kv, batch_idx] # TODO MING
                mQscale_cur = mQscale[batch_idx, None, head_idx, None] # TODO MING
                mKscale_cur = mKscale[batch_idx, None, head_idx_kv, None]

                gK = cute.local_tile(mK_cur, (self.tile_n, self.tile_hdim), (None, 0))
                gV = cute.local_tile(mV_cur, (self.tile_n, self.tile_hdimv), (None, 0))
                gVscale = cute.local_tile(mVscale_cur, (1, self.tile_hdimv,), (None, 0))
                gKscale = cute.local_tile(mKscale_cur, (self.tile_n, 1), (None, 0))
                if const_expr(self.use_tma_Q):
                    gQ = cute.local_tile(mQ_cur, (self.tile_m, self.tile_hdim), (m_block, 0))
                    load_Q, _, _ = copy_utils.tma_get_copy_fn(
                        tma_atom_Q, 0, cute.make_layout(1), gQ, sQ, single_stage=True
                    )
                    gQscale = cute.local_tile(mQscale_cur, (self.tile_m, 1), (m_block, 0))
                    load_Qscale, _, _ = copy_utils.tma_get_copy_fn(
                        tma_atom_Qscale, 0, cute.make_layout(1), gQscale, sQscale, single_stage=True
                    )
                    # load_Vscale = copy_utils.tma_producer_copy_fn(load_Vscale, pipeline_v)
                # TODO: mcast
                ...
                load_Kscale, _, _ = copy_utils.tma_get_copy_fn(
                    tma_atom_Kscale, 0, cute.make_layout(1), gKscale, sKscale
                )
                load_Kscale = copy_utils.tma_producer_copy_fn(load_Kscale, pipeline_k)

                load_Vscale, _, _ = copy_utils.tma_get_copy_fn(
                    tma_atom_Vscale, 0, cute.make_layout(1), gVscale, sVscale
                )
                load_Vscale = copy_utils.tma_producer_copy_fn(load_Vscale, pipeline_v)
                # load_Vscale = None
                ...
                if const_expr(not self.use_block_sparsity):
                    ...
                    pipeline_k.producer_acquire(
                        kv_producer_state,
                        extra_tx_count=(self.tma_copy_bytes["Q"] + self.tma_copy_bytes["Qscale"])
                        if const_expr(self.use_tma_Q)
                        else 0,
                    )
                    if const_expr(self.use_tma_Q):
                        load_Q(tma_bar_ptr=pipeline_k.producer_get_barrier(kv_producer_state))
                        load_Qscale(tma_bar_ptr=pipeline_k.producer_get_barrier(kv_producer_state))
                    load_K(src_idx=n_block, producer_state=kv_producer_state)
                    load_Kscale(src_idx=n_block, producer_state=kv_producer_state)

                    if const_expr(not self.intra_wg_overlap):
                        pipeline_v.producer_acquire(kv_producer_state)
                        load_V(src_idx=n_block, producer_state=kv_producer_state)
                        load_Vscale(src_idx=n_block, producer_state=kv_producer_state)
    ...
    def mma(
        ...
        wg_mma_pv = tiled_mma_pv_rs.get_slice(warp_group_thread_layout(warp_group_idx))
        ...
        if const_expr(self.mma_pv_is_rs):
            ...
            tOrP = cute.make_fragment(
                utils.convert_layout_acc_frgA_fp8(cute.make_layout(acc_S_shape)), cutlass.Float8E4M3FN
            )
        # else:
        #     tOrP = tiled_mma_pv.make_fragment_A(wg_mma_pv.partition_A(sP))
        tOrP = tiled_mma_pv_rs.make_fragment_A(wg_mma_pv.partition_A(sP))
        tOrVt = tiled_mma_pv_rs.make_fragment_B(wg_mma_pv.partition_B(sVt))

        # ///////////////////////////////////////////////////////////////////////////////
        # Smem copy atom tiling
        # ///////////////////////////////////////////////////////////////////////////////
        smem_copy_atom_P = utils.get_smem_store_atom(self.arch, cute.Float8E4M3FN)
        ...
        acc_O_int32 = cute.recast_tensor(acc_O, Int32)
        acc_O.fill(0.0)
        # acc_tmp= cute.make_fragment(acc_shape_O, Float32)
        acc_tmp = None
        smem_copy_params = SimpleNamespace(smem_thr_copy_P=smem_thr_copy_P, tPsP=tPsP)

        mma_qk_fn = partial(
            sm90_utils.gemm_zero_init, tiled_mma_qk, (self.tile_m, self.tile_n), tSrQ, tSrK, int8=self.int8
        )
        mma_pv_fn = partial(sm90_utils.gemm_w_idx, tiled_mma_pv_rs, acc_O_int32 if self.int8 else acc_O, tOrP, tOrVt)
    ...
    def mma_one_n_block(
        ...
        pipeline_v: cutlass.pipeline.PipelineAsync,
        acc_O: cute.Tensor,
        acc_O_int32: cute.Tensor,
        acc_tmp: cute.Tensor | None,
        tOrP: cute.Tensor,
        sVt: cute.Tensor,
        sQscale: cute.Tensor | None,
        sKscale: cute.Tensor | None,
        sVscale: cute.Tensor | None,
        sPscale: cute.Tensor | None,
        smem_copy_params: SimpleNamespace,
        softmax: Softmax,
        ...
        # S = Q @ K.T
        acc_S0 = mma_qk_fn(B_idx=smem_pipe_read.index, wg_wait=-1)
        self.warp_scheduler_barrier_arrive()

        warp_tidx = cute.arch.thread_idx()[0] % 32
        ...
        scale_a = cute.make_fragment(cute.make_layout((2,1,1)), cute.Float32)
        scale_a[0] = sQscale[r_0]
        scale_a[1] = sQscale[r_1]
        scale_a_SSA0 = scale_a.load()
        scale_a_SSA = cute.TensorSSA(scale_a_SSA0.ir_value(), ((1,2,1),1,1), cute.Float32).broadcast_to(acc_S0.shape)

        # print("acc_S0.shape: ", acc_S0.shape, " ", acc_S0.shape[0][2])
        scale_b = cute.make_fragment(cute.make_layout((2,acc_S0.shape[0][2])), cute.Float32)
        for j in cutlass.range_constexpr(acc_S0.shape[0][2]):
                scale_b[j * 2] = sKscale[(col_idx * 2 + j * 8, smem_pipe_read.index)]
                scale_b[j * 2 + 1] = sKscale[(col_idx * 2 + j * 8 + 1, smem_pipe_read.index)]
        scale_b_SSA0 = scale_b.load()
        scale_b_SSA = cute.TensorSSA(scale_b_SSA0.ir_value(), ((2,1,acc_S0.shape[0][2]),1,1), cute.Float32).broadcast_to(acc_S0.shape)

        warpgroup.wait_group(0)
        pipeline_k.consumer_release(smem_pipe_read)
        acc_S_ssa = acc_S0.load().to(cute.Float32) * scale_a_SSA * scale_b_SSA

        # apply mask in a hack way
        if is_first_n_block:
            inf_mask = cute.where(scale_b_SSA0 == cute.Float32(0), -cute.Float32.inf, scale_b_SSA0)
            inf_mask = cute.where(inf_mask == -cute.Float32.inf, inf_mask, cute.Float32(0))
            inf_mask_SSA = cute.TensorSSA(inf_mask.ir_value(), ((2,1,acc_S0.shape[0][2]),1,1), cute.Float32).broadcast_to(acc_S0.shape)
            acc_S_ssa += inf_mask_SSA
        # acc_S_ssa = cute.where(scale_b_SSA != cute.Float32(0), acc_S_ssa,  -cute.Float32.inf)

        if const_expr(self.int8 == True):
            acc_S = cute.recast_tensor(acc_S0, cute.Float32)
        else:
            acc_S = acc_S0
        acc_S.store(acc_S_ssa)

        # if cute.arch.thread_idx()[0] == 128 and cute.arch.block_idx()[0] == 0 and cute.arch.block_idx()[1] == 0 and cute.arch.block_idx()[2] == 0 and n_block == 0:
        #     for i in range(8):
        #         cute.printf("scale_b[%d] = %f %f", i, scale_b[i * 2], scale_b[i * 2 + 1])


        # print("acc_tmp shape:", acc_tmp.shape)
        # for i in cutlass.range_constexpr(16):
        #     # scale_b0 = sVscale[(col_idx * 2 + i * 8, smem_pipe_read.index)]
        #     # scale_b1 = sVscale[(col_idx * 2 + i * 8 + 1, smem_pipe_read.index)]
        #     acc_tmp[(0,0,i), 0, 0] *= scale_a[0] 
        #     acc_tmp[(0,1,i), 0, 0] *= scale_a[1] 
        #     acc_tmp[(1,0,i), 0, 0] *= scale_a[0] 
        #     acc_tmp[(1,1,i), 0, 0] *= scale_a[1] 
        ...
        # compute scale
        P_max_SSA = acc_S.load().reduce(cute.ReductionOp.MAX, init_val=1e-4, reduction_profile=((1,None,1),None,None))
        P_max_SSA = quack_utils.warp_reduce(P_max_SSA, cute.arch.fmax, width=4)

        if const_expr(self.int8 == True):
            P_max_SSA = P_max_SSA * cute.Float32(1.0 / 127.0)
        else:
            P_max_SSA = P_max_SSA * cute.Float32(1.0 / 448.0)

        P_scale_broadcast = cute.TensorSSA(P_max_SSA.ir_value(), ((1,2,1),1,1), P_max_SSA.dtype).broadcast_to(acc_S.shape)

        one_over_P_scale = cute.Float32(1.0) / P_scale_broadcast
        acc_S_fp8_SSA = (acc_S.load() * one_over_P_scale)

        if const_expr(self.int8 == True):
            # acc_S_fp8_SSA = cute.where(acc_S_fp8_SSA > 127., 127., acc_S_fp8_SSA)
            # acc_S_fp8_SSA = cute.where(acc_S_fp8_SSA < -127., -127., acc_S_fp8_SSA)
            # round the fp32 values to nearest integer
            # tmp = cute.make_fragment_like(acc_S, cutlass.Float32)
            # tmp.store(acc_S_fp8_SSA)
            # for i in cutlass.range_constexpr(cute.size(tmp)):
            #     tmp[i] = utils.round_fp32_rn(tmp[i])
            tOrP_cur = cute.make_fragment_like(acc_S, cute.Int8)
            # tOrP_cur.store(tmp.load().to(cutlass.Int8))
            # tOrP_cur.store(acc_S_fp8_SSA.to(cutlass.Int32).to(cutlass.Int8))

            acc_S.store(acc_S_fp8_SSA)


            for i in cutlass.range_constexpr(cute.size(tOrP_cur)):
                tOrP_cur[i] = utils.cvt_u8x1_f32_magic_0_127(acc_S[i])

            # dst_i16 = cute.recast_tensor(tOrP_cur, cutlass.Int16)
            # for i in cutlass.range_constexpr(cute.size(dst_i16)):
            #     dst_i16[i] = utils.cvt_u8x2_f32_magic_0_127(acc_S[i*2], acc_S[i*2 + 1])

            # dst_i16 = cute.recast_tensor(tOrP_cur, cutlass.Int32)
            # for i in cutlass.range_constexpr(cute.size(dst_i16)):
            #     dst_i16[i] = utils.cvt_u8x4_f32_magic_0_127(acc_S[i*4], acc_S[i*4 + 1], acc_S[i*4 + 2], acc_S[i*4 + 3])

        else:
            acc_S.store(acc_S_fp8_SSA)
            tOrP_cur = cute.make_fragment_like(acc_S, cute.Float8E4M3FN)

            dst_i16 = cute.recast_tensor(tOrP_cur, cutlass.Int16)
            for i in cutlass.range_constexpr(cute.size(dst_i16)):
                dst_i16[i] = utils.cvt_f8x2_f16(acc_S[i*2], acc_S[i*2 + 1], cute.Float16)

        # print("acc_S shape: ", acc_S.shape, "stride: ", acc_S.stride)

        # if cute.arch.thread_idx()[0] == 128 and cute.arch.block_idx()[0] == 0 and cute.arch.block_idx()[1] == 0 and cute.arch.block_idx()[2] == 0:
        #     for i in range(16):
        #         cute.printf("tOrP_cur[%d] = %f\n", i, tOrP_acc_fp16[i])


        if const_expr(self.mma_pv_is_rs):
            tPrP = smem_copy_params.smem_thr_copy_P.retile(tOrP_cur)
            # print("tPrP shape: ", tPrP.shape, "stride: ", tPrP.stride)
            # print("tOrP_cur shape: ", tOrP_cur.shape, "stride: ", tOrP_cur.stride)
            cute.copy(smem_copy_params.smem_thr_copy_P, tPrP, smem_copy_params.tPsP)
        softmax.rescale_O(acc_O, row_scale)
        P_scale_broadcast2 = cute.TensorSSA(P_max_SSA.ir_value(), ((1,2,1),1,1), P_max_SSA.dtype).broadcast_to(acc_O.shape)
        # one_over_P_scale2 = cute.Float32(1.0) / P_scale_broadcast2
        # acc_O_ssa = acc_O.load() * one_over_P_scale2
        # acc_O.store(acc_O_ssa)
        acc_O_tmp_SSA = acc_O.load()
        # acc_O_int32.fill(0)
        if const_expr(self.mma_pv_is_rs):
        ...
        # if cute.arch.thread_idx()[0] == 128 and cute.arch.block_idx()[0] == 0 and cute.arch.block_idx()[1] == 0 and cute.arch.block_idx()[2] == 0 and n_block == 0:
        #     # sVscale_tmp = cute.make_fragment(cute.make_layout(128), cute.Float8E4M3FN)
        #     # for i in range(128):
        #     #     sVscale_tmp[i] = sVscale[(i, smem_pipe_read.index)]
        #     for i in range(128):
        #         cute.printf("sVscale[%d] = %f", i, sVscale[(i, smem_pipe_read.index)])
        # O += P @ V
        mma_pv_fn(B_idx=smem_pipe_read.index, wg_wait=-1)

        # print("acc_O shape:", acc_O.shape)

        scale_b2 = cute.make_fragment(cute.make_layout((2,acc_O.shape[0][2])), cute.Float32)
        for i in cutlass.range_constexpr(acc_O.shape[0][2]):
            scale_b2[i * 2] = sVscale[(col_idx * 2 + i * 8, smem_pipe_read.index)]
            scale_b2[i * 2 + 1] = sVscale[(col_idx * 2 + i * 8 + 1, smem_pipe_read.index)]
        scale_b2_SSA0 = scale_b2.load()
        P_scale_broadcast = cute.TensorSSA(scale_b2_SSA0.ir_value(), ((2,1,acc_O.shape[0][2]),1,1), cute.Float32).broadcast_to(acc_O.shape) * P_scale_broadcast2

        warpgroup.wait_group(0)

        if const_expr(self.int8 == True):
            acc_O_ssa = acc_O_int32.load().to(cute.Float32) * P_scale_broadcast
        else:
            acc_O_ssa = acc_O.load() * P_scale_broadcast
        acc_O.store(acc_O_ssa + acc_O_tmp_SSA)
