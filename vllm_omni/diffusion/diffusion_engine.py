...
class DiffusionEngine:
    ...
    async def step(self, request: OmniDiffusionRequest) -> list[OmniRequestOutput]:
        ...
        submitted = SubmittedDiffusionRequest(
            sched_req_id=request.request_ids[0] if request.request_ids else "",
            request=request,
    ...
    def sync_add_req_and_wait_for_response(self, request: OmniDiffusionRequest) -> DiffusionOutput:
        with self._rpc_lock:
            ...
            while True:
                ...
                # NOTE: sync_add_req_and_wait_for_response() is synchronous, will be only called
    ...
    def _dummy_run(self):
        ...
        output = self.sync_add_req_and_wait_for_response(request)
        if output.error:
    ...
    def close(self) -> None:
        ...
        if hasattr(self, "worker_thread") and self.worker_thread.is_alive():
            ...
            if self.worker_thread.is_alive():
                logger.warning("Worker thread did not terminate within 10s.")
